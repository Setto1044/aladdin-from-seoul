...
public class HouseServiceImpl implements HouseService {
    ...
    // l1 cache
    private final Cache<String, List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> caffeineCache;

    //l2 cache
    private final RedisTemplate<String, List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> redisTemplate;
    ...
    public HouseSummaryNearbyResponseDto getHouseSummaryNearby(HouseSummaryNearbyRequestDto requestDto) {
        List<String> gridKeys = getCoveredGridKeys(
                ...
                requestDto.getDistanceMeter()
        );

        // L1 확인
        Map<String, List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> l1Hits = new HashMap<>();
        List<String> l1Misses = new ArrayList<>();

        for (String key : gridKeys) {
            List<HouseSummaryNearbyResponseDto.HouseSummaryDto> l1 = caffeineCache.getIfPresent(key);
            if (l1 != null) {
                log.info("✅ L1 hit: {}", key);
                l1Hits.put(key, l1);
            } else {
        ...
        // L2 multiGet
        Map<String, List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> l2Hits = new HashMap<>();
        List<String> l2Misses = new ArrayList<>();

        if (!l1Misses.isEmpty()) {
            List<List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> l2Values =
                    redisTemplate.opsForValue().multiGet(l1Misses);

            for (int i = 0; i < l1Misses.size(); i++) {
                String key = l1Misses.get(i);
                List<HouseSummaryNearbyResponseDto.HouseSummaryDto> val = l2Values.get(i);
                if (val != null) {
                    log.info("✅ L2 hit: {}", key);
                    l2Hits.put(key, val);
        ...
        Map<String, List<HouseSummaryNearbyResponseDto.HouseSummaryDto>> l3Results = new HashMap<>();

        for (String key : l2Misses) {
            List<HouseSummaryNearbyResponseDto.HouseSummaryDto> data = loadFromDb(key);
        ...
        // 결과 합산
        List<HouseSummaryNearbyResponseDto.HouseSummaryDto> result = gridKeys.stream()
                .flatMap(key -> {
        ...
        return HouseSummaryNearbyResponseDto.of(result);
    }

    private List<HouseSummaryNearbyResponseDto.HouseSummaryDto> loadFromDb(String gridKey) {
