...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiResponseDto> handleBoardCreationException(BoardCreationException e) {
		return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDto.of(false, "등록에 실패했습니다.", e.getMessage()));
	}

	@ExceptionHandler(ResourceNotFoundException.class)
	public ResponseEntity<ApiResponseDto> handleResourceNotFoundException(ResourceNotFoundException e) {
		return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDto.of(false, "리소스를 찾을 수 없습니다.", e.getMessage()));
	}
	...
	public ResponseEntity<ApiResponseDto> handleGlobalException(Exception e) {
		return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDto.of(false, "서버 오류가 발생했습니다.", e.getMessage()));
	}

	@ExceptionHandler(RuntimeException.class)
	public ResponseEntity<ApiResponseDto> handleRuntimeException(RuntimeException e) {
		return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDto.of(false, e.getMessage()));
	}
