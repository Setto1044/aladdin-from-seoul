...
- 대화 시작 시 자기소개: "제가 멋진 장소를 추천해드리죠." 외 금지
...
### 예시 질문
1. 자연 환경보다 편의시설이 더 가까운 지역을 선호하나요?
2. 조용한 주거지보다 핫플레이스가 많은 지역을 선호하나요?
...
"당신에게 딱 맞는 장소: [사용자 질문에 가장 부합하는 1개 서울 자치구]"
`
      }
      ...
      if (this.answers.length >= 6) {
        return `${answerHistory}\n지금까지의 답변을 바탕으로 "당신에게 딱 맞는 장소: [서울의 자치구명]" 형식으로 결과만 알려주세요.`
      } else {
        return `${answerHistory}\n다음 한 개의 질문을 "~~보다 ~~를 선호하나요?" 형식으로만 작성해주세요. 이전에 질문한 옵션은 다시 묻지 마세요. 50자를 넘지 않아야 합니다.`
      }
    },
    handleAnswer(answer) {
        ...
        const response = await axios.post(
          'https://api.openai.com/v1/chat/completions',
          {
            ...
            messages: [
              ...
              {
                role: 'user',
                content: `${answerHistory}\n이 답변들을 바탕으로 가장 적합한 서울시 자치구를 추천해주세요. "당신에게 딱 맞는 장소: [서울의 자치구명]" 형식으로 응답해주세요.`,
              },
            ],
            max_tokens: 100,
            temperature: 0.7,
